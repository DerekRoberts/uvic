 ...
 */
	private DrawableBTNode<E> buildTree( TreeNode<E> top, int level ) {
		...
		return new DrawableBTNode<E>( top.element,
			buildTree( top.leftChild, level+1 ),
			buildTree( top.rightChild, level+1 ),
			level );
	}
...
/*
		public String toString() {
			...
			if (left == null) {
				...
				s +="left child: "+left.element;
			}
			if (right == null) {
				s += "\tright child is null";
			} else {
				s += "\tright child: "+right.element;
			}
			return s;
		}
*/	
